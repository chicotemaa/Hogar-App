...
export const BodyOT = ({ Formulario }: Props) => {
  ...
  const [pages, setPages] = React.useState<{ pagina: number, modulos: [] }[]>()
  ...
  React.useEffect(() => {
    ...
    const paginador = crearPaginador(Formulario.propiedadModulos)
    setPages(paginador)
  ...
  return (
    <View style={styles.container}>
      <View>
        <Text>{pages ? ('Si hay formulario') : ('No hay formulario')}</Text>
      </View>
